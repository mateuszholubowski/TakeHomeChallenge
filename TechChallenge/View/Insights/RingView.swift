...
struct RingView: View {
    ...
    private func offset(for categoryIndex: Int) -> Double {
        ...
        let prevOffset = offset(for: categoryIndex - 1)
        let prevRatio = ratio(for: categoryIndex - 1)
        return prevOffset + prevRatio
    }

    private func gradient(for categoryIndex: Int) -> AngularGradient {
        let color = viewModel.allCategories[categoryIndex].color
    ...
    private func percentageText(for categoryIndex: Int) -> String {
        let ratio = self.ratio(for: categoryIndex)
    ...
    var body: some View {
        ZStack {
            ForEach(viewModel.allCategories.indices) { categoryIndex in
                PartialCircleShape(
                    offset: offset(for: categoryIndex),
                    ratio: ratio(for: categoryIndex)
                )
